...
public class Main {
    public static void main(String[] args) {
        ...
        studentArrayList.add(new Student("Константин", 14));
        studentArrayList.add(new Student("Аркадий", 14));
...
//        Used Iterator to avoid ConcurrentModificationException if doing remove
//        Iterator<Student> it = studentArrayList.iterator();
//        while (it.hasNext()) if (it.next().age>18) it.remove();
        studentArrayList.removeIf(student -> student.age > 18);
        for(Student s : studentArrayList) System.out.println(s.name +", "+ s.age);

        System.out.println("\nудалить с помощью removeIf всех студентов чьё имя содержит букву 'к'");
        studentArrayList.removeIf(student -> student.name.contains("к") || student.name.contains("К"));
        for(Student s : studentArrayList) System.out.println(s.name +", "+ s.age);

        System.out.println("\nвсем студентам добавить к возврасту 10 лет с помощью forEach");
        studentArrayList.forEach(student -> student.age+=10);
        for(Student s : studentArrayList) System.out.println(s.name +", "+ s.age);
